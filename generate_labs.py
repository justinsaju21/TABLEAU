...
LABS = [
  ('0','lab0.html','Orientation & Setup','Part 0','Install Tableau Public and VS Code, explore the Superstore dataset, and understand how the XML evaluator works.',
   ...
   ['Go to **public.tableau.com/s/download**, create a free account, and install Tableau Public on your computer. During installation accept all defaults. Launch the application once installation completes.',
     'On the Tableau Public start screen click **File > Open** and download the Sample Superstore Excel file from: tableau.com/solutions/workbook/superstore. Open it — Tableau will load the Data Source tab automatically.',
     'In the **Data Source tab** examine the three sheets: Orders, People, Returns. Drag the **Orders** sheet to the canvas area. Notice how Tableau displays field names, data types (Abc, #, calendar icon), and row counts in the preview grid.',
     'Click **Sheet 1** at the bottom to open the worksheet view. Spend a few minutes dragging fields to Rows and Columns shelves. Drag **Category** to Columns, **Sales** to Rows — Tableau auto-creates a bar chart. Undo after exploring.',
     'Go to **File > Save As (Local)**. Choose a folder you control, name the file "Exp0_Setup.twb", and save. Open the saved file in VS Code (right-click > Open With > VS Code) and observe the XML tags: `<workbook>`, `<datasources>`, `<worksheets>`. Then upload the .twb file here.']),
  ...
  ('1','lab1.html','Connecting and Preparing Data','Part 1','Connect to Superstore, rename fields, change datatypes, and inspect XML structure in VS Code.',
   [('check-1','Datasource Connected','TableauEvaluator.hasNode(xml,"datasource")','25'),
    ...
    ('check-3','Renamed Fields','TableauEvaluator.hasNode(xml,"column")','25'),
    ('check-4','Geographic Role Assigned','TableauEvaluator.hasDrillPath(xml,3)','25')],
   ['Open Tableau, click **Microsoft Excel** under Connect, and browse to your Superstore file. Drag the **Orders** sheet onto the canvas. Verify you see a green tick on the data source name and a preview of rows in the lower pane.',
     ...
     'In the **Data pane**, right-click the **State** field, go to **Geographic Role > State/Province**. A small globe icon will appear on the field confirming the role assignment. Save the workbook as "Exp1_Data.twb" and upload it here.']),
...
{steps_content}
    ...
    <div class="panel">
      ...
      <div class="panel-body">
        ...
        <div class="upload-zone" id="ssZone" style="height:100px;border-style:dashed;background:rgba(255,255,255,0.02)" title="Click to upload screenshot">
