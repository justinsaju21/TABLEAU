...
LABS = [
  ...
  ('2','lab2.html','Basic Visualization and Formatting','Part 1','Create Bar, Line, and Scatter charts with proper labels and formatting.',
   [('check-1','3 Worksheets Present','xml.getElementsByTagName("worksheet").length >= 3','20'),
    ('check-2','Bar Chart Mark','(function(x){var ss=x.toLowerCase().split(\'<worksheet\');for(var s of ss){var cm=s.match(/<cols>([^<]*)<\\/cols>/);var rm=s.match(/<rows>([^<]*)<\\/rows>/);var c=cm?cm[1]:\'\';var r=rm?rm[1]:\'\';if((c.includes(\'none:category\')||c.includes(\'none:sub-category\'))&&(r.includes(\'sum:sales\')||r.includes(\'sum:profit\')))return true;if(r.includes(\'none:category\')&&c.includes(\'sum:sales\'))return true;}return false;})(xmlString)','20'),
    ('check-3','Line Chart Mark','(function(x){var ss=x.toLowerCase().split(\'<worksheet\');for(var s of ss){var cm=s.match(/<cols>([^<]*)<\\/cols>/);var rm=s.match(/<rows>([^<]*)<\\/rows>/);var c=cm?cm[1]:\'\';var r=rm?rm[1]:\'\';if(c.includes(\':order date\')&&(r.includes(\'sum:profit\')||r.includes(\'sum:sales\')))return true;}return false;})(xmlString)','20'),
    ('check-4','Scatter Plot Mark','(function(x){var ss=x.toLowerCase().split(\'<worksheet\');for(var s of ss){var cm=s.match(/<cols>([^<]*)<\\/cols>/);var rm=s.match(/<rows>([^<]*)<\\/rows>/);var c=cm?cm[1]:\'\';var r=rm?rm[1]:\'\';if(c.includes(\'sum:sales\')&&r.includes(\'sum:profit\'))return true;if(c.includes(\'sum:profit\')&&r.includes(\'sum:sales\'))return true;}return false;})(xmlString)','20'),
    ('check-5','Labels Present','xmlString.toLowerCase().includes(\'mark-labels-show\')','20')],
  ...
  ('6','lab6.html','Statistical Visualization Lab','Part 2','Build Histograms, Box Plots, Bubble Charts, and Treemaps.',
   [('check-1','Histogram (Bar)','(function(x){var ss=x.toLowerCase().split(\'<worksheet\');for(var s of ss){var cm=s.match(/<cols>([^<]*)<\\/cols>/);var rm=s.match(/<rows>([^<]*)<\\/rows>/);var c=cm?cm[1]:\'\';var r=rm?rm[1]:\'\';if((c.includes(\'none:category\')||c.includes(\'none:sub-category\'))&&(r.includes(\'sum:sales\')||r.includes(\'sum:profit\')))return true;if(r.includes(\'none:category\')&&c.includes(\'sum:sales\'))return true;}return false;})(xmlString)','20'),
    ('check-2','Box Plot (Circle)','(function(x){var ss=x.toLowerCase().split(\'<worksheet\');for(var s of ss){var cm=s.match(/<cols>([^<]*)<\\/cols>/);var rm=s.match(/<rows>([^<]*)<\\/rows>/);var c=cm?cm[1]:\'\';var r=rm?rm[1]:\'\';if(c.includes(\'sum:sales\')&&r.includes(\'sum:profit\'))return true;if(c.includes(\'sum:profit\')&&r.includes(\'sum:sales\'))return true;}return false;})(xmlString)','20'),
  ...
  ('11','lab11.html','Geospatial Analytics Laboratory','Part 4','Build Filled Maps and Symbol Maps, and use MAKEPOINT/DISTANCE calculations.',
   [('check-1','Filled Map','TableauEvaluator.hasMarkType(xml,"Map")','25'),
    ('check-2','Symbol/Circle Map','(function(x){var ss=x.toLowerCase().split(\'<worksheet\');for(var s of ss){var cm=s.match(/<cols>([^<]*)<\\/cols>/);var rm=s.match(/<rows>([^<]*)<\\/rows>/);var c=cm?cm[1]:\'\';var r=rm?rm[1]:\'\';if(c.includes(\'sum:sales\')&&r.includes(\'sum:profit\'))return true;if(c.includes(\'sum:profit\')&&r.includes(\'sum:sales\'))return true;}return false;})(xmlString)','25'),
  ...
  ('15','lab15.html','Data Pipeline Integration (Bonus)','Part 5 — Optional','Generate a CSV with Python and build a live monitoring dashboard. Bonus +10 marks.',
   [('check-1','Valid Workbook','xml !== null','34'),
    ...
    ('check-3','Line Chart (Time Series)','(function(x){var ss=x.toLowerCase().split(\'<worksheet\');for(var s of ss){var cm=s.match(/<cols>([^<]*)<\\/cols>/);var rm=s.match(/<rows>([^<]*)<\\/rows>/);var c=cm?cm[1]:\'\';var r=rm?rm[1]:\'\';if(c.includes(\':order date\')&&(r.includes(\'sum:profit\')||r.includes(\'sum:sales\')))return true;}return false;})(xmlString)','33')],
...
<script src="../js/evaluator.js?v=20260506_2"></script>
<script src="../js/tutor.js?v=20260506_2"></script>
</head>
<body>
<div class="topbar">
